...
- **Tight coupling to `tigress/core`** — constants `TRANSLATIONS`, `SYSTEM_ROOT`, `CONFIG`, and function `__()` must be available at runtime. The library cannot be used standalone.
- **ICS escaping** (`escapeIcsText`) applies `str_replace` to literal `\n` and `\r` strings rather than actual newline characters. This means actual newlines in user-provided text are not escaped, which can produce invalid ICS output.
- **`addAttachment` path handling** — the method unconditionally appends `/` to `$path` if it doesn't end with one, then concatenates `$path . $name`. This means `$path` is treated as a directory and `$name` as the filename. If `$name` already contains a full path, the result may be mangled.
- **`MicrosoftGraph::getEventIdByICalUId`** uses the raw `$iCalUId` in a query filter without sanitization, which could cause issues with special characters in UIDs.
- **`isLocationFree`** always uses `$locationAddresses[0]` as the calendar owner for the `getSchedule` call regardless of how many addresses are passed. This is likely correct for the API but could be surprising.
- **`Smartschool::sendMail`** writes the web services password into the database (`pwd_web_services` column) in plain text — a security concern.
- **`Smartschool::sendMail`** uses `header('Location: ...')` for error redirects — this assumes HTTP context and has side effects (exit not called after redirect).
- **No explicit dependency on `tigress/core` in `composer.json`** — it only appears as a `require-dev` entry, yet the library will not function without it at runtime.
